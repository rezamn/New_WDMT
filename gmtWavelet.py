...
def frequency_band(input_signal, sampling_frequency):
    from colors import load_gmt_colors
    import random
    pygmt.config(
    ...
    region = [1 / 3 / T, 2 ** max_j * 4 / 3 / T, 0, .3]
    colors = load_gmt_colors()
    for i in range(max_j):
        fig.plot(
            region=region,
            projection="X15cl/3c",
            frame=["S", "xaf+lFrequency(Hz)"],
            x=[2 ** i / 3 / T, 2 ** i * 4 / 3 / T],
            y=[.05*(i % 3 + 1) , .05*( i % 3 + 1)],
            pen="1p,%s" % (colors[random.randint(0,461)])
        )
        fig.text(
            x=((2 ** i / 3 / T) + (2 ** i * 4 / 3 / T))/2,
            y=.05*(i % 3 + 1.4),
            text="j = %d, f = [%.2f %.2f]" % (i, 2 ** i / 3 / T, 2 ** i * 4 / 3 / T),
            font="5p,Times-Roman",
            justify="CM"
        )
